...
const TestimonialsPage = ({ videoUrl, title, testimonialImg }) => {
  ...
  const getYouTubeEmbedUrl = (url) => {
    ...
    // Regular expression to handle various YouTube URL formats (standard, share, embed)
    const regExp = /^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|&v=)([^#&?]*).*/;
    const match = url.match(regExp);
    const videoId = (match && match[2].length === 11) ? match[2] : null;

    // Return the embed URL with autoplay enabled
  ...
  return (
    <section className="w-full py-12 md:py-16 lg:py-20 px-4 md:px-6 bg-gray-50 flex flex-col items-center relative overflow-hidden z-10">
      ...
      {/* Video Container with Enhanced Animation & Effects */}
      <motion.div
        className="relative w-full max-w-5xl rounded-3xl overflow-hidden shadow-2xl border-4 border-white/50 bg-black"
        initial={{ opacity: 0, scale: 0.95 }}
        whileInView={{ opacity: 1, scale: 1 }}
        viewport={{ once: true, margin: "-100px" }}
        transition={{ duration: 0.8, ease: "easeOut" }}
      ...
      >
        {/* Decorative Border Animation */}
        <motion.div
          className="absolute inset-0 rounded-3xl border-2 border-[#ED0331]/30 pointer-events-none z-20"
          animate={{
            opacity: [0.3, 0.6, 0.3],
          }}
          transition={{
        ...
        <AnimatePresence mode="wait">
          {isPlaying ? (
            <motion.div
              key="video"
              className="relative w-full aspect-video" // Added aspect-video for proper 16:9 ratio
              initial={{ opacity: 0, scale: 0.99 }}
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              transition={{ duration: 0.5 }}
            >
              {/* Check if it's a YouTube URL or a standard video file */}
              {youTubeEmbedUrl ? (
                <iframe
                  src={youTubeEmbedUrl}
              ...
              {/* Optional: Removed the overlay info when playing because it blocks YouTube controls */}
            </motion.div>
          ) : (
            <motion.div
              key="thumbnail"
